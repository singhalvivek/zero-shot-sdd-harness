...
- **LLM provider + model:** Google Gemini via `google-genai`, per-request `model_id` ∈ {`gemini-2.5-flash`, `gemini-2.5-pro`}. Key `AGENT_GEMINI_API_KEY` (pydantic settings, `env_prefix=AGENT_`) passed explicitly to `genai.Client(api_key=...)`.
  > **Model-id verification — BLOCKED at scaffold:** verification against the live Gemini API could not complete because `AGENT_GEMINI_API_KEY` in `.env` does not authenticate (HTTP 401 UNAUTHENTICATED on a direct `generativelanguage.googleapis.com` call; the value is 53 chars and does not start with `AIza`). `gemini-2.5-flash` / `gemini-2.5-pro` remain the intended GA ids; the runner constructs `GeminiProvider` per-request with the chosen `model_id` (the skeleton's `DEFAULT_MODEL` placeholder is NOT used). The exact ids MUST be re-verified once a valid Developer API key is supplied, before the Phase 1 gate.
